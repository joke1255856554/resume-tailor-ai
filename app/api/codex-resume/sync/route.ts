...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.sessionId || !body.factBank || !body.jobDescription?.trim()) return NextResponse.json({ error: '缺少 workspace 同步数据。' }, { status: 400 })
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Codex workspace 同步失败。' }, { status: 500 })
  }
}
